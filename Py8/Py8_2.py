...
with open('city.list.json', 'rb') as json_file:
    for line in json_file:
        ...
        with sqlite3.connect(db_filename) as conn:
            conn.execute('''
                insert into weather (id_города, Город, Страна, Дата, Температура, id_погоды) VALUES (?,?,?,?,?,?)''', (
                city_id, name, country, date, temp, temp_id
            )
        )
conn.commit()
conn.close()
...
for country in countries:
    print(country, end=',')
user_country = input("Введите обозначение страны из списка выше\n")
...
with sqlite3.connect(db_filename) as conn:
    ...
    for item in json_line2['list']:
        ...
        with sqlite3.connect(db_filename) as conn:
            ...
            if cur.fetchall():
                conn.execute("""update weather set Дата = ?, Температура = ?, id_погоды = ? where id_города = ?""",
                     (date, temp, temp_id, city_id))
                print('updated succesfully:  ', date, temp, temp_id, name, country)
            else:
                conn.execute("""insert into weather (id_города, Город, Страна, Дата, Температура, id_погоды) VALUES (?,?,?,?,?,?)""",
                         (city_id, name, country, date, temp, temp_id))
                print('inserted succesfully:  ', city_id, name, country, date, temp, temp_id )
